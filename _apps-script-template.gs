...
// First order = the account has never PAID a deposit yet. An unpaid/pending order does NOT count.
function isFirstOrderFor(accountId) {
  var orders = sheetObjects("Orders");
  var paid = { "Paid":1, "Packed":1, "Out":1, "Returned":1, "Extended":1, "Lost/Damaged":1 };
  for (var i = 0; i < orders.length; i++) {
    var o = orders[i];
    if (String(o.accountId) !== String(accountId)) continue;
    if (String(o.paymentStatus) === "Confirmed" || paid[String(o.status)]) return false;
  }
  return true;
}

// A customer may have only ONE pending order. Cancel any earlier still-unpaid pending
// order from this account and release its reserved copies (a new checkout supersedes it).
function supersedePendingOrders(accountId) {
  var sh = sheet("Orders"); if (!sh) return;
  var data = sh.getDataRange().getValues(); var head = data[0].map(String);
  var iAcc = head.indexOf("accountId"), iStatus = head.indexOf("status"),
      iPay = head.indexOf("paymentStatus"), iIsbn = head.indexOf("isbns");
  for (var r = 1; r < data.length; r++) {
    if (String(data[r][iAcc]) === String(accountId) &&
        String(data[r][iStatus]) === "Pending payment" &&
        String(data[r][iPay]) !== "Confirmed") {
...
function login(b) {
  ...
  return { ok: true, accountId: c.id, name: c.name, isFirstOrder: isFirstOrderFor(c.id), pending: pendingItemsFor(c.id) };
}
// The account's current unpaid pending order's books (lets checkout merge/trim the two lists)
function pendingItemsFor(accountId) {
  var orders = sheetObjects("Orders");
  for (var i = 0; i < orders.length; i++) {
    var o = orders[i];
    if (String(o.accountId) === String(accountId) && String(o.status) === "Pending payment" && String(o.paymentStatus) !== "Confirmed") {
      var titles = String(o.titles || "").split(";").map(function (x) { return x.trim(); });
      var isbns = splitIsbns(o.isbns);
      var items = [];
      for (var j = 0; j < isbns.length; j++) items.push({ isbn: isbns[j], title: titles[j] || "" });
      return items;
    }
  }
  return [];
...
function createOrder(b) {
  ...
  // one pending order per customer: cancel any earlier unpaid pending order first
  supersedePendingOrders(c.id);
